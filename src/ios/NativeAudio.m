...
NSString* INFO_PLAYBACK_RESUME = @"(NATIVE AUDIO) Resume";
...
- (void) preloadComplex:(CDVInvokedUrlCommand *)command
{
    ...
    [self.commandDelegate runInBackground:^{
        if (existingReference == nil) {
            ...
            } else if ([[NSFileManager defaultManager] fileExistsAtPath : assetPath]) {
...
- (void) pause:(CDVInvokedUrlCommand *)command
{
    ...
    if ( audioMapping ) {
        ...
        } else if ([audioMapping allKeys].count != 0) {
            for(NSString* audioID in audioMapping) {
                NSObject* asset = audioMapping[audioID];
                NSLog(@"[pause] all, asset %@", audioID);
                if ([asset isKindOfClass:[NativeAudioAsset class]]) {
                    NativeAudioAsset *_asset = (NativeAudioAsset*) asset;
                    [_asset pause];
...
- (void) resume:(CDVInvokedUrlCommand *)command
{
    ...
    NSArray* arguments = command.arguments;
    NSString *audioID = [arguments objectAtIndex:0];
    NSLog(@"[resume] %@", audioID);

    [self.commandDelegate runInBackground:^{
        if (audioMapping) {
            NSLog(@"[resume] audiomapping: %@", audioMapping);
            NSObject* asset = audioMapping[audioID];
            if (asset != nil){
                NSLog(@"[resume] asset %@", audioID);
                if ([asset isKindOfClass:[NativeAudioAsset class]]) {
                    NativeAudioAsset *_asset = (NativeAudioAsset*) asset;
                    [_asset resume];

                    NSString *RESULT = [NSString stringWithFormat:@"%@ (%@)", INFO_PLAYBACK_RESUME, audioID];
                    [self.commandDelegate sendPluginResult:[CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString: RESULT] callbackId:callbackId];
                ...
                for(NSString* audioID in audioMapping) {
                    NSObject* asset = audioMapping[audioID];
                    NSLog(@"[resume] all, asset %@", audioID);
                    if ([asset isKindOfClass:[NativeAudioAsset class]]) {
                        NativeAudioAsset *_asset = (NativeAudioAsset*) asset;
                        [_asset resume];
